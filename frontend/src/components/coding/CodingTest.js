...
const CodingTest = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await axios.get(`${API}/api/ai/coding-questions`); // use full URL for testing
...
const submitAnswers = async (finalAnswers) => {
  setLoading(true);
  try {
    const res = await axios.post(`${API}/api/ai/evaluate-coding`, { answers: finalAnswers });
